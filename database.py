...
# Add station
def add_station(stationname, stationtypeid, latitude, longitude):
    conn = database_connect()
    if conn is None:
        return None
    cur = conn.cursor()
    try:
        sql = """
        INSERT INTO opaltravel.Stations (stationname, stationtypeid, latitude, longitude)
        VALUES (%s, %s, %s, %s);
        """
        cur.execute(sql, (stationname, stationtypeid, latitude, longitude))
        conn.commit()
        cur.close()
        conn.close()
...
# Delete station by stationtypeid
def delete_station(stationtypeid):
    conn = database_connect()
    if conn is None:
        return None
    cur = conn.cursor()
    try:
        sql = f"""
        DELETE FROM opaltravel.Stations
        WHERE stationtypeid = %s;
        """
        cur.execute(sql, (stationtypeid,))
        conn.commit()
        cur.close()
        conn.close()
...
# Update station
def update_station(stationid, stationname, stationtypeid, latitude, longitude):
    conn = database_connect()
    if conn is None:
        return None
    cur = conn.cursor()
    try:
        sql = """
        UPDATE opaltravel.Stations
        SET stationname = %s, stationtypeid = %s, latitude = %s, longitude = %s
        WHERE stationid = %s;
        """
        cur.execute(sql, (stationname, stationtypeid, latitude, longitude, stationid))
        conn.commit()
        cur.close()
        conn.close()
...
def list_stations():
    ...
    try:
        ...
        print(f"Error showing all stations: {e}")
        cur.close()
        conn.close()
        return None

def show_stations_by_coordinates(lower_latitude, upper_latitude, lower_longitude, upper_longitude):
    conn = database_connect()
    if conn is None:
        return None
    cur = conn.cursor()
    try:
        ...
        """
        cur.execute(sql, (lower_latitude, upper_latitude, lower_longitude, upper_longitude))
        result = cur.fetchall()
        cur.close()
        conn.close()
        return result
    except Exception as e:
        print(f"Error showing stations by location: {e}")
        cur.close()
        conn.close()
        return None

def count_stations_by_type():
    conn = database_connect()
    if conn is None:
        return None
    cur = conn.cursor()
    try:
        sql = """
        SELECT StationTypes.stationtypeid AS id, StationTypes.stationtypename AS name, COUNT(Stations.stationid) as count
        FROM opaltravel.StationTypes
        LEFT JOIN opaltravel.Stations USING(stationtypeid)
        GROUP BY id, name
        ...
        print(f"Error counting stations by type: {e}")
        cur.close()
        conn.close()
        return None

def list_stationtype():
    # Get the database connection and set up the cursor
    conn = database_connect()
    if(conn is None):
        # If a connection cannot be established, send an Null object
